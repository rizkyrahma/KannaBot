...
let date = global.botdate
    ...
    let namapasangan = global.db.data.users[`${pasangan}`].name
    ...
    orderTitle: `▮Menu ▸`,
    thumbnail: await (await fetch(fla + 'Inv')).buffer(), //Gambarnye
...
⛔ *Banned:* No
`
    conn.send2Button(m.chat, str, wm, `⋮☰ Back`, `.menu`, `Shop`, `.shop`, ftrol, { contexInfo: { mentionedJid: [global.db.data.users[m.sender].pasangan] }})
}
